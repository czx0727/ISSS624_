...
From the various models, it seems to suggest that the doubly constrained model is the best. With the lowest RMSE and the best fit plot of MORNING_PEAK against TRIPS.

However, the coefficient of the different variables are removed (N/A). Further investigation is needed to further determine any collinearity between the variables.

In this aspect, Origin Constraint is the best model to be used.

We observed that the coefficient is the largest for TRAIN_COUNT, followed by SCHOOL_COUNT, then BUSINESS_COUNT.

One reason may be passengers transit between MRT and buses, in which the number of passengers who alighted from MRT will affect the number of passengers who are taking buses.

For schools, parents may be bringing their children to school. Business, surprisingly, has a less impact compared to schools, but also accounts for passengers who are going to work between 6am to 9am.

HDB_Count is of a less impact as probably passengers may walk a certain distance to another bus stop to take the required bus to their destination.
