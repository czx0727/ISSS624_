...
Exploratory Spatial Data Analysis (ESDA) hold tremendous potential to address complex problems facing society. In this study, I will demonstrate how to apply appropriate Local Indicators of Spatial Association (GLISA) to undercover the spatial and spatio-temporal mobility patterns of public bus passengers in Singapore.
...
***Passenger Volume by Origin Destination Bus Stops*** will be downloaded from [LTA DataMall](https://datamall.lta.gov.sg/content/datamall/en.html). Three months dataset are downloaded: **Aug, Sep and Oct 23**. This study will use only Aug 23 data.
...
-   ***hexagon***, a [hexagon](https://desktop.arcgis.com/en/arcmap/latest/tools/spatial-statistics-toolbox/h-whyhexagons.htm) layer of 250m (this distance is the perpendicular distance between the centre of the hexagon and its edges.). Guidance on how it is created is located [here](https://urbandatapalette.com/post/2021-08-tessellation-sf/). It will be demonstrated in the Section [4.2 Hexagon].
...
busstop and origin6_9 dataset are combined by left join with the unique identifer: "ORIGIN_PT_CODE" in origin6_9 and "BUST_STOP_N" ain busstop
...
Next, I will make the combine dataset a SF
...
```{r}
# cell size of 250
area_honeycomb_grid = st_make_grid(test_points_1, c(250, 250), what = "polygons", square = FALSE, crs = 3414)
...
Before moving to the next step, I save output into rds format.

```{r}
write_rds(busstop_mpsz, "data/rds/busstop_mpsz.rds")  
```

Append the hexagon from busstop_mpsz data frame onto origin6_9 data frame.
...
Before continue, it is a good practice to check for duplicating records. The code chunk below shows how it is done.
...
Notably, Joo Koon station exhibits a substantial influx of passengers, potentially stemming from the conclusion of work hours and the discharge of full-time national servicemen. This trend is similarly observed in stations proximate to Gedong Camp. Furthermore, heightened passenger activity is evident at Woodlands Checkpoint, Johor Bahru Checkpoint, and the Senoko region.
...
Append the hexagon from busstop_mpsz data frame onto origin17_20 data frame. The steps will be the same as above.
...
The Choropleth Visualization shows a decrease in passenger volumes in comparison to the two preceding plots. Likewise, there is a notable concentration of high activity around central zones such as Orchard and Somerset stations. This increase in numbers might be linked to families gathering for brunch or lunch.

It's worth mentioning that Joo Koon station experiences lower passenger volumes on weekends, as fewer individuals are commuting to work in the Tuas area.
...
2.  Orchard and Somerset stations consistently record high passenger volumes, except during the weekday morning peak hours (6 am to 9 am). This high passenger volumes might be due to individuals traveling to central locations for their meals.
...
Majority of the hexagon layer is not significant. However, there are a few layers of p value that is significant.
...
From the plot above, low-low are observed in areas such as Tuas, Jurong, areas near Gedong Camp. In addition, high-high are observed in areas such as Woodlands, Dover area.
...
From the plot above, low-low are observed in areas such as Tuas, Jurong, areas near Gedong Camp as well. In addition, high-high are observed in areas such Woodlands.
